# Running unsigned SimpleCue builds
...
SimpleCue listens on the network, so the first time it starts Windows shows:

> **Windows Defender Firewall has blocked some features of SimpleCue**
...
domain. SimpleCue needs this to receive OSC, MIDI Show Control, MMC and Art-Net/sACN control.

If you deny it (or the prompt appears behind another window and times out), incoming show control is silently dropped — cues will only fire from the local UI
...
**Private**. If SimpleCue isn't listed, **Allow another app…** → **Browse** to the `.exe`.

Or from an elevated PowerShell:

```powershell
New-NetFirewallRule -DisplayName "SimpleCue" -Direction Inbound -Program "C:\Path\To\SimpleCue.exe" -Action Allow -Profile Private,Domain
```

To remove a bad rule and get the prompt back on next launch:

```powershell
Get-NetFirewallRule -DisplayName "*SimpleCue*" | Remove-NetFirewallRule
```
...
```sh
codesign --force --deep --sign - "/Applications/SimpleCue.app"
```

Verify it took:

```sh
codesign -dv --verbose=4 "/Applications/SimpleCue.app"
spctl -a -vv "/Applications/SimpleCue.app"   # still reports "rejected" — ad-hoc is not notarization
```
